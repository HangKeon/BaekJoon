...
ll init(ll s, ll e, ll node)	//구간 합 트리 만드는 함수 -> init(시작 인덱스, 끝 인덱스, 구하는 노드)
{
	...
	return tree[node] = (init(s, mid, node * 2) * init(mid + 1, e, node * 2 + 1)) % 1000000007;	//왼쪽 자식 노드 + 오른쪽 자식 노드
}

ll mul(ll s, ll e, ll node, ll l, ll r)	//구간 곱 구하는 함수 ->mul(시작 인덱스, 끝 인덱스, 구하는 노드, 구간 합 범위)
{
	...
	return (mul(s, mid, node * 2, l, r) * mul(mid + 1, e, node * 2 + 1, l, r)) % 1000000007;	//왼쪽 자식 노드 * 오른쪽 자식 노드
}
